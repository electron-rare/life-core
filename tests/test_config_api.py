...
def test_platform_exposes_sse_flag(client, monkeypatch):
    """F4L_UI_FEATURE_SSE=false should surface as ui_features.sse=false."""
    monkeypatch.setenv("F4L_UI_FEATURE_SSE", "false")
    ...
    assert resp.status_code == 200
    flags = resp.json()["ui_features"]
    assert "sse" in flags
    assert flags["sse"] is False
